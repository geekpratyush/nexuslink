...
| Fork it, change it and open a pull request | [`CONTRIBUTING.md`](CONTRIBUTING.md) |
| Read the API reference for a class or module | [`javadoc/`](javadoc/index.html) — generated by `mvn -Pjavadoc javadoc:aggregate` |
...
| Take it into my organisation and wire it into our library pipeline | [`PIPELINE.md`](PIPELINE.md) — the handover document |
...
- `index.html` is the landing page — self-contained, no external CSS, fonts or JS, with a parallax
  hero and sections for install, what is inside, documentation, contributing and troubleshooting.
- `doc.html?d=<file>.md` renders any of the markdown documents in this folder in the site's own
  styling, so the reader never lands on raw markdown. The list of documents it will serve is the
  `DOCS` map at the top of its script — add a document there when you add one here.
- `assets/` holds the logo system: `nexuslink-mark.svg` (the icon), the full lockups used by
  `README.md`, and `favicon.svg`.
- `javadoc/` is the generated API reference, committed so Pages can serve it. Regenerate it with
  `mvn -Pjavadoc javadoc:aggregate` whenever the public API moves.
