...
| **5** | Enterprise messaging (JMS, IBM MQ, Solace, MQTT, RabbitMQ, cloud) | ✅ **MQTT** (live) + **RabbitMQ** (dashboard + DLX) + **AWS SQS/SNS** (full UI + DLQ redrive, live vs LocalStack) + **JMS** (service live vs Artemis + `JmsView` send/receive/browse UI, wired into MainWindow) + **IBM MQ** + **Solace** + **Google Pub/Sub** + **Azure Service Bus** (queues + topics/subs + DLQ, emulator-aware) done, **MQTT on Paho v5** (v5 properties + persistent message history); all §5 client families shipped — only per-provider tails remain (JMS JNDI/provider-JAR, RabbitMQ shovel/federation) |
| **6** | Advanced HTTP (gRPC, GraphQL) | ✅ **Complete** — **gRPC** (reflection, unary, **pure `.proto` parser**, **server/client/bidi streaming panel**) + **GraphQL** (query/introspection + **schema explorer** + **subscriptions over `graphql-transport-ws`**) |
...
| **8** | Databases & enterprise (JDBC, **Mongo**, Redis, LDAP, SSH, SNMP) | ✅ JDBC (+TLS, sortable/filterable grid + export + **visual query builder + EXPLAIN + in-grid/structure editing**) + Mongo + **Redis** (+ **cluster + Sentinel**) + **LDAP** (search + filter builder + entry CRUD + LDIF + DIT tree) + **SNMP** (v1/v2c/v3 USM on the wire + MIB names + trap/inform receiver) + **SSH terminal** (MINA SSHD PTY shell + custom VT100/xterm renderer with xterm-256 + alt-screen + local port forwarding) done, plus **Mongo drop-index + auth/users panel (§8.3)** and **LDAP StartTLS (§8.4)** — **Phase 8 complete** |
...
**Overall: ~90% of in-scope tasks complete** (276 done · 25 in-progress · 7 not started by top-level
checkbox — see `TASKS.md`). **Every phase 0–9 has its feature work complete.** What is left is not a
phase: 7 unstarted shell/polish items (2× `module-info.java`, `ProtocolView` SPI, `PropertiesPanel`,
`ProfileEditorDialog`, bundled Inter/JetBrains-Mono fonts, REST history sidebar) plus 25 `[-]` tails
where the feature works and a named sub-option is outstanding.
...
Current plan, being worked across sessions using
