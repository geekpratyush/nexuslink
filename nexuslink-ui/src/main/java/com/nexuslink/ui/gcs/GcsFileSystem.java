...
public final class GcsFileSystem implements FileSystem, FileTransfer {
    ...
    @Override public byte[] readFile(FileItem item, long maxBytes) {
        return service.getObjectBytes(ObjectPath.bucket(item.path()), ObjectPath.prefix(item.path()),
                (int) Math.min(maxBytes, Integer.MAX_VALUE));
    }

    @Override public void writeFile(String dir, String name, byte[] data) {
        service.putObject(ObjectPath.bucket(dir), ObjectPath.childKey(dir, name), data, null);
    }
    ...
    @Override public void upload(Path localFile, String remoteDir, String destName, LongConsumer progress) throws Exception {
        ...
        service.uploadFile(bucket, ObjectPath.childKey(remoteDir, destName), localFile, progress);
    }
