...
specification (`NexusLink_Specification.md`). As of the latest session, roughly **45% of the
tracked tasks are done** (~112 done · ~24 in-progress · ~116 not started). `TASKS.md` is the
live, phase-by-phase tracker and the source of truth; the table below summarizes it.
...
| Certificate manager · environment-variable system | ⏳ Not started |
...
| MQTT · RabbitMQ · JMS · IBM MQ · Solace · cloud messaging (SQS/SNS/Service Bus/Pub-Sub) | ⏳ Not started |
...
> **Short answer to "is it done?": no.** The foundation, the help/vault/history infrastructure,
> and the most-used protocols (REST/WS/SSE/GraphQL/gRPC/SQL/Mongo/Redis/object-storage/MCP/LLM)
> are built and many are verified live. Enterprise messaging (MQTT/RabbitMQ/JMS/MQ), directory
> services (LDAP/SSH/SNMP), the certificate manager, environment variables, monitoring/metrics,
...
nexuslink-security          Credential vault (AES-256-GCM, PBKDF2)
...
nexuslink-protocol-messaging  (reserved — JMS/MQTT/RabbitMQ, not yet implemented)
